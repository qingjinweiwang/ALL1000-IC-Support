...
#define	WriteStatusCmd2		0x31
#define	WriteStatusCmd3		0x11
...
uChar ReadStatusData2(void);
uChar ReadStatusData3(void);
...
void WriteStatusRegister(uChar cmd, uChar data);
...
uChar ReadFun(void)
{
	...
	{	/* if special bit block then read boot lock status */
		...
		reg1 = DevWordData.all;
		SRAMPointer = 0x00;
		SaveSRAMByteData(DevWordData.all);
		Delay_1ms(100);

		DevWordData.all = ReadStatusData2();
		reg2 = DevWordData.all;
		SRAMPointer = 0x01;
		SaveSRAMByteData(DevWordData.all);
		Delay_1ms(100);

		DevWordData.all = ReadStatusData3();
		reg3 = DevWordData.all;
		SRAMPointer = 0x02;
		SaveSRAMByteData(DevWordData.all);
		Delay_1ms(100);

		SRAMPointer = 0x03;
		SaveSRAMByteData(reg1);
		SaveSRAMByteData(reg2);
		SaveSRAMByteData(reg3);
		return SUCCESS;
	}
	...
	{ /* Program main flash */
		...
		while (1)
		{
			...
			else
			{
				...
				while (1)
				{
					StatusData = ReadStatusData() & StatusWIPBit; /* only check bit0, checking all socket & get check result */
...
uChar VerifyFun(void)
{
	...
	{	/* if special bit block then read boot lock status */
		...
		DevByteData = ReadStatusData() & PrtRegMaskBit; /* checking all socket & get check result */
...
uChar EraseFun(void)
{
	...
	if (IfFirstTimeEntry())
	{
		...
		SetCSPinStatus(0); /* set chip CS port to Vil */
		Delay_10nsX(10);
		ArmSpi_TxReg() = BulkEraseCmd; /* send cmd Byte */
		SPIRWDelay();
...
{
	uChar RamWordData, RamWordData2, RamWordData3, PollingTimeCnt, StatusData;
	...
	SRAMPointer = 0x01;
	RamWordData2 = GetSRAMByteData();

	SRAMPointer = 0x02;
	RamWordData3 = GetSRAMByteData();

	WriteStatusRegister(WriteStatusCmd, RamWordData);
	WriteStatusRegister(WriteStatusCmd2, RamWordData2);
	WriteStatusRegister(WriteStatusCmd3, RamWordData3);
	PollingTimeCnt = 200;
	while (1)
	{
		StatusData = ReadStatusData() & PrtRegMaskBit; /* only check bit0, checking all socket & get check result */
...
uChar ReadStatusData(void)
{
	SetCSPinStatus(0);
	Delay_10nsX(10);
	ArmSpi_TxReg() = ReadStatusCmd; /* send cmd Byte */
	SPIRWDelay();

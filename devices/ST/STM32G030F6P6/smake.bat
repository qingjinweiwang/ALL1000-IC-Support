...
REM ==============================================
REM Step 5: Copy BIN and SPC files to target directory
REM ==============================================
echo.
echo [5/5] Copying files to target directory...
set "TARGET_DIR=C:\Program Files (x86)\Hi-Lo\ALL1000\Algo1"
set "TARGET_BIN=%TARGET_DIR%\%PROJECT_NAME%.bin"
set "TARGET_SPC=%TARGET_DIR%\%PROJECT_NAME%.spc"
set "SOURCE_SPC=%PROJECT_ROOT%\%PROJECT_NAME%.spc"

echo Target directory: "%TARGET_DIR%"
echo Target BIN file: "%TARGET_BIN%"
echo Target SPC file: "%TARGET_SPC%"
...
echo   Deployed SPC file: "%TARGET_SPC%"
echo ----------------------------------------------------------------
...
REM Copy the SPC file
if exist "%SOURCE_SPC%" (
    echo Copying "%SOURCE_SPC%" to "%TARGET_SPC%"...
    copy "%SOURCE_SPC%" "%TARGET_SPC%"
    if errorlevel 1 (
        echo Warning: Failed to copy SPC file!
    ) else (
        echo [OK] SPC file copied successfully to target directory
    )
) else (
    echo Warning: SPC file not found: "%SOURCE_SPC%"
)

/*
V1.0 2026-08-31
1. First version for MICRON M25PE40 (Datasheet Rev.D, 01/2018).
...
5. Support Page Program (02h), Page Write (0Ah), Page Erase (DBh),
   4KB Subsector Erase (20h), 64KB Sector Erase (D8h) and Bulk Erase (C7h).
6. Support 8-bit Status Register (05h/01h), BP2-BP0 protection control.
*/
/******************************************************************************\
...
#define PageEraseCmd            0xDB  /* 3-byte address, erase one 256-byte page */
#define SubsectorEraseCmd       0x20  /* 3-byte address, erase one 4KB subsector */
#define SectorEraseCmd          0xD8  /* 3-byte address, erase one 64KB sector */

#define BulkEraseCmd            0xC7  /* Bulk erase entire 512KB main array */
#define WriteLockRegCmd         0xE5  /* 3-byte address + 1-byte Lock Register write */
#define ReadLockRegCmd          0xE8  /* 3-byte address + 1-byte Lock Register read */
#define ReadIDCmd               0x9F  /* Read JEDEC ID, M25PE40 = 20h-80h-13h */
#define DeepPowerDownCmd        0xB9  /* Enter Deep Power-Down mode */
#define ReleasePowerDownCmd     0xAB  /* Release from Deep Power-Down mode */
...
uChar CurrSRData ;
	...
	if( IfFirstTimeEntry() )
	{
		/* M25PE40 Bulk Erase is allowed only when BP2-BP0 are all zero. */
		CurrSRData = ReadStatusData() ;
		if( CurrSRData & LSB(PrtRegMaskBit) )
		{
...
uChar CurrSRData ;
    ...
    /*
     * SRAM[0x00]：上位机下发的 BP2-BP0 目标值。
     * 读-改-写只修改 BP2-BP0,保留状态寄存器其余位。
     */
    PtrData(GEnSelSckNum) = (uChar)(~GangErrSckMask) ; /* enable all pass socket before gang operation */
    Delay_10nsX(3) ;
    ...
    CurrSRData = ReadStatusData() ;
    SR1Data |= ( CurrSRData & (uChar)(~LSB(PrtRegMaskBit)) ) ;

    /**********************************************************
     * 06h Write Enable
     * 01h Write Status Register：写一个状态寄存器字节。
     **********************************************************/
    if( !WriteStatusWELBit() )
    {
    ...
    PtrData(OneDataCmd) = SR1Data ;             /* S7-S0：BP2-BP0 */
    SPIRWDelay() ;
...
uChar CurrSRData ;
    ...
    PtrData(OneDataCmd) = SR1Data ;             /* S7-S0：BP2-BP0=0 */
    SPIRWDelay() ;
...
/******************* Secure Subroutine ***********************\
Notes: M25PE40 Lock Register support is not implemented.

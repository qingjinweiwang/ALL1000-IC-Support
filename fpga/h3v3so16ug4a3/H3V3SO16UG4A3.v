...
module	H3V3SO16UG4A3(
			...
			/////////////// Socket A signal //////////////////////////
					...
					CSPortA,RstPortA,WPPortA,HoldPortA,CS2PortA,
					GndPortA,VccPortA,

			/////////////// Socket B signal //////////////////////////
					SDIPortB,SDOPortB,SCKPortB,
					CSPortB,RstPortB,WPPortB,HoldPortB,CS2PortB,
					GndPortB,VccPortB,

			/////////////// Socket C signal //////////////////////////
					SDIPortC,SDOPortC,SCKPortC,
					CSPortC,RstPortC,WPPortC,HoldPortC,CS2PortC,
					GndPortC,VccPortC,

			/////////////// Socket D signal //////////////////////////
					SDIPortD,SDOPortD,SCKPortD,
					CSPortD,RstPortD,WPPortD,HoldPortD,CS2PortD,
					GndPortD,VccPortD,
...
/********************************************************************/
input wire SDOPortA ;
output wire SDIPortA,SCKPortA,CSPortA,HoldPortA,CS2PortA ;
output wire RstPortA,WPPortA ;
inout wire GndPortA ;
inout wire VccPortA ;
...
/********************************************************************/
input wire SDOPortB ;
output wire SDIPortB,SCKPortB,CSPortB,HoldPortB,CS2PortB ;
output wire RstPortB,WPPortB ;
inout wire GndPortB ;
inout wire VccPortB ;
...
/********************************************************************/
input wire SDOPortC ;
output wire SDIPortC,SCKPortC,CSPortC,HoldPortC,CS2PortC ;
output wire RstPortC,WPPortC ;
inout wire GndPortC ;
inout wire VccPortC ;
...
/********************************************************************/
input wire SDOPortD ;
output wire SDIPortD,SCKPortD,CSPortD,HoldPortD,CS2PortD ;
output wire RstPortD,WPPortD ;
inout wire GndPortD ;
inout wire VccPortD ;
...
initial
  begin
	...
	GDevMaskData <= 16'b1 ;
	EnSelSocketNum <= EnSckACtrlFlag ; /* set default to read socket A data */
	  ...
	  begin
		if( RdWrFlag == ReadData ) /* read data from device */
		  begin
			...
			else if( (ClockStep >= 6) && (ClockStep <= 8) )
			  begin
				DataRegA[6] <= SDOPortA ;
				DataRegB[6] <= SDOPortB ;
				DataRegC[6] <= SDOPortC ;
			...
			else if( (ClockStep >= 12) && (ClockStep <= 14) )
			  begin
				DataRegA[5] <= SDOPortA ;
				DataRegB[5] <= SDOPortB ;
				DataRegC[5] <= SDOPortC ;
			...
			else if( (ClockStep >= 18) && (ClockStep <= 20)  )
			  begin
				DataRegA[4] <= SDOPortA ;
				DataRegB[4] <= SDOPortB ;
				DataRegC[4] <= SDOPortC ;
			...
			else if((ClockStep >= 24) && (ClockStep <= 26)  )
			  begin
				DataRegA[3] <= SDOPortA ;
				DataRegB[3] <= SDOPortB ;
				DataRegC[3] <= SDOPortC ;
			...
			else if( (ClockStep >= 30) && (ClockStep <= 32)  )
			  begin
				DataRegA[2] <= SDOPortA ;
				DataRegB[2] <= SDOPortB ;
				DataRegC[2] <= SDOPortC ;
			...
			else if( (ClockStep >= 36) && (ClockStep <= 38)  )
			  begin
				DataRegA[1] <= SDOPortA ;
				DataRegB[1] <= SDOPortB ;
				DataRegC[1] <= SDOPortC ;
			...
			else if( (ClockStep >= 42) && (ClockStep <= 44)  )
			  begin
				DataRegA[0] <= SDOPortA ;
				DataRegB[0] <= SDOPortB ;
				DataRegC[0] <= SDOPortC ;
...
always @(posedge FPGAGCLK0) /* Scan Clock = 66MH/2 = 33MHz */
  begin /* Nor(ADM) Flash Logic Process >=15ns, So MUST use <= 33MHz Scan Clock */
	...
	if( ClockStep > 48) /* re-initial wave-step to first status */
	  begin
...
assign CSPortA = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[0]) ? 1'b1 : CSReg[0]) ; /* when error disable device! */
assign CS2PortA = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[0]) ? 1'b1 : CSReg[1]) ; /* when error disable device! */
assign RstPortA = ((StateReg==IdleState)||(GangErrSckMask[0])) ? 1'bz : RstReg ;
...
assign CSPortB = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[1]) ? 1'b1 : CSReg[0]) ; /* when error disable device! */
assign CS2PortB = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[1]) ? 1'b1 : CSReg[1]) ; /* when error disable device! */
assign RstPortB = ((StateReg==IdleState)||(GangErrSckMask[1])) ? 1'bz : RstReg ;
...
assign CSPortC = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[2]) ? 1'b1 : CSReg[0]) ; /* when error disable device! */
assign CS2PortC = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[2]) ? 1'b1 : CSReg[1]) ; /* when error disable device! */
assign RstPortC = ((StateReg==IdleState)||(GangErrSckMask[2])) ? 1'bz : RstReg ;
...
assign CSPortD = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[3]) ? 1'b1 : CSReg[0]) ; /* when error disable device! */
assign CS2PortD = (StateReg==IdleState) ? 1'bz : ((GangErrSckMask[3]) ? 1'b1 : CSReg[1]) ; /* when error disable device! */
assign RstPortD = ((StateReg==IdleState)||(GangErrSckMask[3])) ? 1'bz : RstReg ;
